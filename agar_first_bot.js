...
(function (g, q) {
  ...
  function Ba(a) {
    ...
    function getAllFood() {
        ...
        elementList = getListBasedOnFunction(
          function (element){
            for (var i = 0; i < m.length; i++) {
                if (v[element].id != m[i].id // not me
                  && !v[element].isVirus && (v[element].size * 1.25 <= m[i].size)  || (v[element].size <= 11)){return true;} else{return false;}
                }
            },
        v);
    ...
    function findDestination() {
        ...
        if (m[0] != null) {
            ...
            if (allPossibleNiceViruses.length != 0) {
                closestNiceViruse = [allPossibleNiceViruses[0], allPossibleNiceViruses[0].size*computeDistance(allPossibleNiceViruses[0].x, allPossibleNiceViruses[0].y, m[0].x, m[0].y)];

                for (var i = 1; i < allPossibleNiceViruses.length; i++) {
                    var testD = allPossibleNiceViruses[i].size*computeDistance(allPossibleNiceViruses[i].x, allPossibleNiceViruses[i].y, m[0].x, m[0].y)
                    if (testD < closestNiceViruse[1]) {
                        closestNiceViruse = [allPossibleNiceViruses[i], testD];
                    }
                }

                console.log("Going After: " + closestNiceViruse[0].name);
            }
